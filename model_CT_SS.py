...
class Unet_CT_SS(object):
    ...
    def train(self, train_from_scratch=False, pretrained_weights=None):
            
        model = self.createModel()
        
        if not train_from_scratch and pretrained_weights is not None:
            print('-'*30)
        ...
        for epochs in range(self.numEpochs):
            ...
            for number, each in enumerate(os.listdir(train_images_path)):
                print('case',number+1,': ', each, '\n Epoch #', epochs+1, '/', self.numEpochs)
                ...
                acc = acc + history.history['accuracy'][0]
                loss=loss+history.history['loss'][0]
            if epochs in [1,2,3,4,6,7,9,14,24,29,34,44,49,74,79,90,99]:
    ...
    def train3D(self, train_from_scratch=False, pretrained_weights=None):
    ...
    #                    wImg = self.volumeBasedWeighting(trainLabels)
            ...
            if epochs in [1,3,9,14,24,29,34,44,49,74,79,90,99,124,139,149,174,189,199]:
                model.save_weights(os.path.join(self.save_folder,str(epochs+1)+'_'+self.checkWeightFileName))
                np.save(self.save_folder+'/history.npy',hist)                
#            model.save_weights(os.path.join(self.save_folder,str(epochs+1)+'_'+self.checkWeightFileName))                
